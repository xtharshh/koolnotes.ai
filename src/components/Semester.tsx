  ...
  return (
    <div className=" p-5 text-white">
      <div className="w-full max-w-screen-lg">
        <h1 className="text-3xl dark:text-white text-black font-bold text-center">Semesters</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 custom-lg:grid-cols-4 gap-4 mt-5 justify-center custom-md:grid-cols-4 custom-nmd:grid-cols-2 ">
          {semesters.map((semester, index) => (
            ...
            className="p-2 bg-eclipse bg-cover bg-center shadow-lg text-lg text-white rounded-lg cursor-pointer text-center h-44 w-25 font-bold"
          >
            <h3>{semester.name}</h3> 
          </button>
        ...
        {selectedSemesterIndex !== null && (
          <h2 className="text-center my-5 text-2xl dark:text-white text-black font-bold">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 custom-lg:grid-cols-4 gap-4 mt-5">
              {semesters[selectedSemesterIndex].subjects.map((subject, subjectIndex) => (
                <button 
                  ...
                  className="p-2 bg-eclipse text-white rounded-lg cursor-pointer text-center h-16 w-30 font-bold"
                >
                  <h2>{subject.name}</h2>
                </button>
              ))}
        ...
        {isOpen && selectedSubjectIndex !== null && (
          <div className="fixed dark:bg-black bg-gray-50 text-white p-5 text-center left-1/2 transform -translate-x-1/2 -translate-y-1/2 justify-center z-50 w-4/5 h-2/3 rounded-lg border border-gray-200 overflow-hidden mt-10">
            <button onClick={closeModal} className="absolute top-2 right-2 p-2 dark:bg-black bg-white dark:text-white text-black rounded-lg cursor-pointer font-bold">X</button>
            <div ref={descriptionRef}>
              <h2 className="text-2xl mb-2 dark:text-white text-black font-bold">
                {selectedSemesterIndex !== null && selectedSubjectIndex !== null && semesters[selectedSemesterIndex].subjects[selectedSubjectIndex].name}
              </h2>
              <p className='dark:text-white text-black font-bold'>{selectedSemesterIndex !== null && selectedSubjectIndex !== null && semesters[selectedSemesterIndex].subjects[selectedSubjectIndex].description}</p>
              <div className="mt-5 dark:bg-black">
                {selectedSemesterIndex !== null && selectedSubjectIndex !== null && semesters[selectedSemesterIndex].subjects[selectedSubjectIndex].descriptionButtons?.map((button, index) => (
                  <button
                    ...
                    className="block w-1/2 p-2 my-1 bg-yellow-600 text-white rounded-lg cursor-pointer text-center font-bold"
                  >
                    {button.title}
                  </button>
                ))}
